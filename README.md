...
- **User-based ACL**: Each user has an individual API key mapped to permitted datasources. `config/access_control.yaml` declares OU / classification / role attributes per the fornix ABAC schema; runtime enforcement is active — `confidential` datasources are blocked for `member`/`junior` roles even if accidentally listed.
